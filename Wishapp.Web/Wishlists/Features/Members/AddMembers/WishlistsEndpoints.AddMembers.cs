...
{
    private static async Task<Results<NoContent, NotFound<Error>, BadRequest<Error>, ForbidHttpResult, UnauthorizedHttpResult>> AddMembers(
        [FromRoute] Guid id,
        AddMembersRequest request,
        ClaimsPrincipal user,
        ApplicationDbContext db,
        IAuthorizationService authorizationService,
        ICommandHandler<AddMembersCommand> handler,
        CancellationToken ct)
    {
        ...
        var result = await handler.HandleAsync(
            new AddMembersCommand(id, userIdResult.Value, request.Members), ct);
