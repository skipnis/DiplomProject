...
using Wishapp.Web.Wishlists.Features.Members.GetMembers;

namespace Wishapp.Web.Wishlists;

public static partial class WishlistsEndpoints
{
    private static async Task<Results<Ok<List<WishlistMemberDto>>, ForbidHttpResult, UnauthorizedHttpResult>> GetMembers(
        [FromRoute] Guid id,
        ClaimsPrincipal user,
        ApplicationDbContext db,
        IAuthorizationService authorizationService,
        IQueryHandler<GetMembersQuery, List<WishlistMemberDto>> handler,
        CancellationToken ct)
    {
        ...
        var result = await handler.HandleAsync(new GetMembersQuery(id), ct);
